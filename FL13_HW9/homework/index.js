...
const mapArray = (arr, f) => {
    ...
    const pushToResultArray = x => resultArr.push(f(convertToNumber(x)));
...
const formatDate = (date) => {
    const formatted = number => number < 10 ? '0' + number : number.toString();
...
function isEqual(arr1, arr2) {
    ...
    return arr1.every((arr1Value, index) => arr1Value === arr2[index]);
}